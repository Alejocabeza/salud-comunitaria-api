...
class RequestMedicine(SQLModel, table=True):
    ...
    doctor_id: int = Field(default=None, sa_column=Column('doctor_id', ForeignKey('doctors.id', ondelete='CASCADE')))
...
class UpdateRequestMedicineRequest(SQLModel):
    ...
    doctor_id: Optional[int] = Field(default=None, sa_column=Column('doctor_id', ForeignKey('doctors.id', ondelete='CASCADE')))
    reason: Optional[str] = Field(default=None, max_length=255)
